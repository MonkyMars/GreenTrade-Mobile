...
export default function SellerScreen() {
	...
	const [averageEcoScore, setAverageEcoScore] = useState<number>(seller?.rating || 0);
	...
	const calculateAverageEcoScore = (listings: FetchedListing[]): number => {
		if (listings.length === 0) return seller?.rating || 0;

		const sum = listings.reduce((total, listing) => total + listing.ecoScore, 0);
		const result: number = parseFloat((sum / listings.length).toFixed(1))
	...
	const fetchSeller = async () => {
		try {
			...
			if (data) {
				...
				setAverageEcoScore(calculateAverageEcoScore(data.listings));
				startAnimations();
			} else {
