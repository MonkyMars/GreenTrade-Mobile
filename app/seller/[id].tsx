...
import { fetchSellerData } from 'lib/backend/seller/getSeller'
...
import { useCustomAlert } from 'lib/services/useCustomAlert'
...
export default function SellerScreen() {
	...
	const [loadingSeller, setLoadingSeller] = useState(
		!!params.id,
	)
	...
	const fetchSeller = async (isRefreshing: boolean) => {
		if (isRefreshing) {
			setRefreshing(true)
		}
		if (!params.id) {
		...
		try {
			...
			const sellerData = await fetchSellerData(params.id)
			if (!sellerData) {
				setError('Seller not found')
				return
			}
			setSeller(sellerData.seller)
			setSellerListings(sellerData.listings)
			setAverageEcoScore(calculateAverageEcoScore(sellerData.listings))
		} catch (error) {
	...
	//             ? listing.imageUrl[0]
	//             : (listing.imageUrl as any).urls &&
	//                 Array.isArray((listing.imageUrl as any).urls)
				...
				>
					...
					>
						<View style={{ flexDirection: 'row', alignItems: 'center' }}>
							...
							>
								<Text
									style={{ color: 'white', fontSize: 28, fontWeight: '600' }}
								>
							...
							<View style={{ flex: 1 }}>
								...
								>
									...
									>
										{' '}
										(avg. of {reviews.length} reviews)
									</Text>
									{!isUser && (
										<Pressable
											onPress={() =>
												navigation.navigate('Reviews', { sellerId: seller.id })
											}
										>
											<Text
												style={{
								...
								>
									...
									>
										Active Since {months[new Date(seller.createdAt).getMonth()]}
					...
					>
						...
						<Text
							style={{
								marginTop: 8,
								fontSize: 16,
								color: seller.bio ? colors.textSecondary : colors.textTertiary,
							}}
						>
							...
							>
								<TouchableOpacity
									style={{
										padding: 8,
										backgroundColor:
											selectedView === 'list' ? colors.highlight : colors.card,
									}}
								...
								<TouchableOpacity
									style={{
										padding: 8,
										backgroundColor:
											selectedView === 'grid' ? colors.highlight : colors.card,
									}}
												...
												<Image
													source={{
														uri: Array.isArray(item.imageUrl)
															? item.imageUrl[0]
															: (item.imageUrl as any).urls &&
																Array.isArray((item.imageUrl as any).urls)
