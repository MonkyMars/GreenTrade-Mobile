...
import { isUrl } from 'lib/functions/url'
...
const tabs: ActiveTab[] = ['profile', 'security', 'delete']
...
export default function AccountScreen() {
	...
	useEffect(() => {
		const fetchUserFavorites = async () => {
			if (!authUser) return;
			try {
				const favorites = await getFavorites(authUser.id)
	...
	useEffect(() => {
		const fetchUserListings = async () => {
			if (!authUser) return;
			try {
				const listings = await getSellerListings(authUser.id)
